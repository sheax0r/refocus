 ...
 */
function handleEvent(eventData, eventTypeName, doNotParse) {
  // console.log('eventData inside the event handler' + JSON.stringify(eventData));
  let j;
  if (doNotParse) {
    j = eventData;
  } else {
...
function getHierarchy(rootSubject, filterString) {
  ...
  //   }
  // });
  const apiPath = `/v1/subjects/Salesforce/newhierarchy`;
  const sampleApiPath = '/v1/samples/Salesforce/hierarchy';
  return new Promise((resolve, reject) => {
    request.get(apiPath)
    .set(REQ_HEADERS)
    .end((error, res) => {
      ...
      if (error) {
        ...
        allSubjects = res.body;  
      }
      request.get(sampleApiPath)
      .send(REQ_HEADERS)
      .end((error, sampRes) => {
        if (error) {
          document.getElementById('errorInfo').innerHTML += 'Failed to GET ' +
          ...
          hierarchyLoadEvent = true;

          allSamples = sampRes.body;
          if (gotLens) {
            hierarchyLoadEvent = new CustomEvent('refocus.lens.hierarchyLoad', {
              detail: { absolutePath: 'Salesforce.' },
            });
             LENS_DIV.dispatchEvent(hierarchyLoadEvent);
            for (const obj of allSubjects) {
              // console.log('subject'+ obj);
              const o = {};
              o.new = obj;
              o.old = obj;
              handleEvent(o, eventsQueue.eventType.INTRNL_SUBJ_ADD, true);
            }
            for (const obj of allSamples) {
              const o ={};
              o.new = obj;
              o.old = obj;
              // console.log('sample'+ obj);
              handleEvent(o, eventsQueue.eventType.INTRNL_SMPL_ADD, true);
            }
          }
        }
      });
  ...
  return getPromiseWithUrl('lens', apiPath, (res) => {
    ...
    if (hierarchyLoadEvent) {
      ...
      for (const obj of allSubjects) {
        ...
        o.new = obj;
        o.old = obj;
        // console.log('subj at getlnes promise' + obj);
        handleEvent(o, eventsQueue.eventType.INTRNL_SUBJ_ADD, true);
      }
      for (const obj of allSamples) {
        const o = {};
        o.new = obj;
        o.old = obj;
        // console.log('samples at getlnes promise' + obj);
        handleEvent(o, eventsQueue.eventType.INTRNL_SMPL_ADD, true);
      }
    }
  });
